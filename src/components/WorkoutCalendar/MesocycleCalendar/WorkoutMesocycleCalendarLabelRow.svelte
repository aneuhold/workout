<script lang="ts">
  import type { WorkoutMesocycleCalendarLabelEntry } from './workoutMesocycleCalendarTypes';

  let { labels }: { labels: WorkoutMesocycleCalendarLabelEntry[] } = $props();

  // Build a sparse array of 7 columns
  const labelsByColumn = $derived.by(() => {
    const result: (WorkoutMesocycleCalendarLabelEntry | undefined)[] = Array.from({ length: 7 });
